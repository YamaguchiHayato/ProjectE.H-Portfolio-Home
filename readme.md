...
## 6.技術的工夫点 
  ...
  - ## <a id = "part6-2"></a>6-2.階層型ステートパターンの導入
   ...
   - ## <a id = "part6-3"></a>6-3.全体のクラス設計と状態遷移の仕組み
     - ## クラス設計
     <a href="Image/Character継承図.png.png" target="_blank">
     <img src="Image/Character継承図.png.png" width="100%">
     </a>


  - ## <a id = "part6-4"></a>6-4.データ駆動によるステージの読み込みシステムの構築
    ## ■ 課題
    ## &emsp; 既存の設計ではステージ事にクラスを作成しており、ステージが増えるたびに同じような処理が増えてしまい、コピーコードが量産されていました。

    ## ■ どう工夫したか
    ## &emsp; ステージの情報を描画と座標やモデルパスなどのデータに分けています。
    ## &emsp; また、ステージ事の固有のデータは構造体にまとめることで、 識別子をうけとるだけでそのステージを読み込めるように設計しています。

    ## ■ 結果
    ## &emsp; ステージが増えてもクラス数は増えずにステージデータの登録のみで運用できる形になりました。
...
## 7.今後の展望
- ## キャラの種類の追加
  ## &emsp; 2～3種類プレイアブルキャラを追加予定です。　

- ## ステージの追加
  ## &emsp; こちらも同様に2～3種類追加予定です。

- ## 階層型ステートパターンにイテレーターパターンを適応。
  ## &emsp; 現状の設計では
