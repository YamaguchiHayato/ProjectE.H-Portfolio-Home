...
- ステージ追加
  ...
  - また、ステージにも特性を追加し、より緊迫感のあるバトルを演出したいと思います。

<br>

- `Job System` を導入した非同期スロードシステム
  - 読み込みと構築の分担は実装済みです。ロード処理をJobとして抽象化し、JobQueueへ登録するJobSystemの導入を予定しています。WorkerThreadによる並列実行を行い、　ロード処理の効率化と拡張性向上を目指します。
   
  - 参考記事URL:  [こちらからアクセスできます](https://taskflow.github.io/taskflow/FAQ.html) 
  

<br>

- Utility AI × StateMachine を組み合わせた意思決定NPCの実装
 
  -  従来のNPCは状態遷移ベースのAIのみで制御していたため、
     複数の行動候補を状況に応じて比較できず、不自然な行動や判断停止が発生しています。
  <br>

  -  そこで、複数の行動を点数化して比較できる  `Utility AI`に着目しました。このシステムを導入することによって、NPCだけどユーザが操作しているような自然な行動を実装したいと考えています。
  <br>

  - 参考記事URL: [こちらからアクセスできます](https://zenn.dev/sanmal/articles/9ed9989c11b7eb?locale=en&utm_source=chatgpt.com) 
